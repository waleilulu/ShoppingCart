...
namespace Project0220.Controllers
{
    public class CartController : Controller
    {
        ...
        public IActionResult AddListToCart(string data)
        {
            ...
            if (IsAuthenticated())
            {
                List<myModels.Product> products = JsonConvert.DeserializeObject<List<myModels.Product>>(data);
                // 獲取已登入用戶的CustomerID
                var memberCookie = HttpContext.Request.Cookies["membercookie"];
                var customerID = _context.Customers
                    .FirstOrDefault(c => c.CustomerId.ToString() == memberCookie)?.CustomerId;
                ...
                if (customerID != null)
                {
                    foreach(myModels.Product product in products) {

                        // 找到現有的 CartItem，包括顏色的比較。如果商品有多個顏色，加入不同顏色商品也會視為不同項目
                        var existingCartItem = _context.CartItems
                            .FirstOrDefault(c => c.CustomerID == customerID && c.ProductID == product.ProductId && c.SelectedColor == product.Color1);
                        ...
                        else
                        {
                            ...
                            var newCartItem = new CartItem
                            {
                                ...
                                Quantity = 1,
                                SelectedColor = product.Color1 // 將選擇的顏色設置到新的 CartItem 中
                            };
